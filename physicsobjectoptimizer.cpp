...
QList<QPolygonF> PhysicsObjectOptimizer::getLineOrPolygonList(const QList<QPointF> &points)
{
    ...
    return m_resultLineOrPolygonList;
}
...
void PhysicsObjectOptimizer::optimizeLine(const QList<QPointF> &points)
{
    ...
    if (m_OptimizedPointList.size() == 1 && minDistance(m_OptimizedPointList.last(), points.at(points.size() - 1))) {
        m_OptimizedPointList << points.at(points.size() - 1);
    } else if (m_OptimizedPointList.size() > 1 && minDistance(m_OptimizedPointList.last(), points.at(points.size() - 1))) {
        m_OptimizedPointList << points.at(points.size() - 1);
    }
...
{
    for( int i = 0; i < ( int ) m_OptimizedPointList.size() - 1; i++ )
    {
        QPolygonF lineShape;
...
void PhysicsObjectOptimizer::createPolygon()
{
    for( int i = 0; i < ( int ) m_OptimizedPointList.size() - 2; i+=3 )
    {
        ...
        polygonShape << QPointF( m_OptimizedPointList.at( i + 0 ).x() / SCALINGFACTOR, m_OptimizedPointList.at( i + 0 ).y() / SCALINGFACTOR );
        polygonShape << QPointF( m_OptimizedPointList.at( i + 1 ).x() / SCALINGFACTOR, m_OptimizedPointList.at( i + 1 ).y() / SCALINGFACTOR );
        polygonShape << QPointF( m_OptimizedPointList.at( i + 2 ).x() / SCALINGFACTOR, m_OptimizedPointList.at( i + 2 ).y() / SCALINGFACTOR );
