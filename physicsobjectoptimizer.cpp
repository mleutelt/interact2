...
OptimizerResult PhysicsObjectOptimizer::getLineOrPolygonList(const QList<QPointF> &points)
{
...
bool PhysicsObjectOptimizer::minDistance(const QPointF &pointOne, const QPointF &pointTwo)
{
    ...
    {
        qCDebug(phoo) << "minDistance: Point " << pointOne << "and " << pointTwo << " too close to each other with distance (" << xDistance << ", " << yDistance << ") (threshold: " << POINTDISTANCE << "))";
        return false;
    }

    qCDebug(phoo) << "minDistance: Point " << pointOne << "and " << pointTwo << " distant enough with (" << xDistance << ", " << yDistance << ") (threshold: " << POINTDISTANCE << "))";
    return true;
}
