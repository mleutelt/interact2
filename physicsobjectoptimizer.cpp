...
void PhysicsObjectOptimizer::createLine()
{
    ...
    for (int i = 0; i < m_OptimizedPointList.count() - 1; i++) {
        ...
        QPointF tmpBoxBottomLeft = QPointF(firstPointTmpRot.x(), firstPointTmpRot.y() - (lineThickness / 2.f));
        QPointF tmpBoxBottomRight =  QPointF(secondPointTmpRot.x(), secondPointTmpRot.y() - (lineThickness / 2.f));
        QPointF tmpBoxTopRight =  QPointF(secondPointTmpRot.x(), secondPointTmpRot.y() + (lineThickness / 2.f));
        QPointF tmpBoxTopLeft =  QPointF(firstPointTmpRot.x(), firstPointTmpRot.y() + (lineThickness / 2.f));

        // qCDebug(phoo) << "createLine: Current line shape before transformation: " << tmpBoxBottomLeft << tmpBoxBottomRight << tmpBoxTopRight << tmpBoxTopLeft;
        ...
        // qCDebug(phoo) << "createLine: Current line shape after transformation: " << lineShape.at(0) << lineShape.at(1) << lineShape.at(2) << lineShape.at(3);
